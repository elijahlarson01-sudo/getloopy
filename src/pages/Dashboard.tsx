...
const Dashboard = () => {
  ...
  const fetchUserData = async (userId: string) => {
    ...
    const { data: progressData, error: subjectProgressError } = await supabase
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {subjects.map((subject) => {
              ...
              return (
                <Card
                  key={subject.id}
                  className="p-6 hover:shadow-lg transition-all border-2 hover:border-primary/30 cursor-pointer"
